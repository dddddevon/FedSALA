"""
generate_cifar100.py — Flexible Pathological Non-IID Data Generator
====================================================================

WHAT THIS DOES:
    Downloads CIFAR-100 and splits it across N clients in a pathological non-IID way.
    Supports:
    ...
    Generates THREE data splits:
      1. Training data  → ../dataset/Cifar100/train/{idx}.npz
      2. Skewed test    → ../dataset/Cifar100/test/{idx}.npz   (same distribution, 1/5 size)
      3. Global test    → ../dataset/Cifar100/test_global/{idx}.npz (all 10 labels, balanced)
...
HOW TO USE:
    ...
    python3 generate_cifar100.py --num_clients 10 --labels_per_client 2 --samples_per_label 1000

    # Ratio-based allocation
    python3 generate_cifar100.py --num_clients 10 --labels_per_client 3 --ratio 4:2:1 --total_per_client 280

    # Camera-trap proxy
    python3 generate_cifar100.py --num_clients 15 --labels_per_client 2 --ratio 6:1 --total_per_client 210

    === JSON Config (Mixed — different client groups) ===

    python3 generate_cifar100.py --config scenarios/hospital_mixed.json
...
OUTPUT:
    Creates ../dataset/Cifar100/{train,test,test_global}/ directories
...
# =========================================================================
# SECTION 1: CIFAR-100 Data Loading
# =========================================================================

def load_cifar100():
    """
    Download CIFAR-100 and organize into per-class pools.
    Returns:
...
def assign_labels_round_robin(client_specs, num_classes=100, seed=42):
    """
    ...
    Args:
        ...
        num_classes: total number of classes (100 for CIFAR-100)
...
def allocate_samples(client_specs, client_labels, train_by_class, test_by_class, 
    ...
    num_clients = len(client_specs)
    class_names = {c: f'c{c}' for c in range(num_classes)}
...
def save_client_data(client_train_data, client_test_data, num_clients, seed):
    ...
    """
    class_names = {c: f'c{c}' for c in range(100)}
    
    output_dir = '../dataset/Cifar100'
...
def generate_whole_label_test_shards(test_by_class, num_clients, num_classes=100, seed=42):
    """
    ...
    # CIFAR-100 test set has 1,000 images per class
...
def generate_flexible_noniid(client_specs, seed=42):
    ...
    # ---- Load CIFAR-100 ----
    train_by_class, test_by_class = load_cifar100()
    
    # ---- Step 1: Assign labels using round-robin with shuffle ----
    print(f"\nPathological Non-IID Split:")
    print(f"  {num_clients} clients")
    
    client_labels = assign_labels_round_robin(client_specs, num_classes=100, seed=seed)
    ...
    generate_whole_label_test_shards(test_by_class, num_clients, num_classes=100, seed=seed)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate CIFAR-100 pathological non-IID data',
...
=== CLI Flags (Uniform — all clients same config) ===
  ...
  python3 generate_cifar100.py --num_clients 10 --labels_per_client 3 --ratio 3:2:1 --total_per_client 300
